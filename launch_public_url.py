# 🌐 LANCEMENT DE L'APPLICATION AVEC URL PUBLIQUE 🌐

# Étape 1 : Sauvegarder l'application dans un fichier
app_code = '''
...
def extract_web_content(url):
    try:
        ...
        return "❌ Erreur lors de l'extraction web"
...
if tool_choice == "🏠 Accueil":
    ...
    audio_file = st.file_uploader("Choisis ton fichier audio", type=['wav', 'mp3', 'flac'])
    ...
    text_input = st.text_area("✍️ Écris ton texte ici :", height=100)
    lang_choice = st.selectbox("🌍 Choisis la langue :", ['fr', 'en', 'es', 'de', 'it'])
    ...
    image_file = st.file_uploader("Choisis ton image", type=['jpg', 'jpeg', 'png', 'gif'])
    ...
    url_input = st.text_input("🔗 Colle ton URL ici :")
    ...
    content_type = st.selectbox("📝 Type de contenu :", 
                               ["Email professionnel", "Post LinkedIn", "Article de blog", "Description produit"])
    
    topic = st.text_input("💡 Sujet/Mots-clés :")
...
Après réflexion, je pense qu'il serait intéressant de discuter de cette opportunité plus en détail.
...
Récemment, j'ai découvert l'importance de {topic} dans notre secteur.
...
#business #innovation #croissance
                """,
                "Article de blog": f"""
# {topic.title()} : Guide Complet

## Introduction

{topic.title()} est devenu un élément essentiel dans notre monde moderne. Dans cet article, nous explorerons les aspects fondamentaux de ce sujet.

## Les Points Essentiels

### 1. Comprendre les bases
La première étape consiste à maîtriser les concepts fondamentaux.

### 2. Mise en pratique
L'application concrète permet de consolider les connaissances.

### 3. Optimisation continue
L'amélioration constante est la clé du succès.

## Conclusion

{topic.title()} offre de nombreuses opportunités pour ceux qui s'y intéressent sérieusement.
                """,
...
Parce qu'il combine performance et fiabilité pour répondre à tous vos besoins.
...
<div style="text-align: center; padding: 2rem; color: #666;">
    🚀 Plateforme IA Gratuite à Vie - Créée avec ❤️ par ton ingénieur IA personnel
</div>
""", unsafe_allow_html=True)
'''
...
# Étape 3 : Importer les outils pour l'URL publique
print("🔧 Importation des outils pour l'URL publique...")
from pyngrok import ngrok
import subprocess
import threading
import time

# Étape 4 : Fonction pour lancer Streamlit
def run_streamlit():
...
print("🚀 Lancement de l'application en arrière-plan...")
...
# Étape 7 : Créer l'URL publique avec ngrok
print("🌐 Création de l'URL publique...")
try:
    ...
    print("="*60)
    print("📋 PARTAGE CETTE URL AVEC TES CLIENTS !")
    print("✅ Ils peuvent l'utiliser GRATUITEMENT, sans installation")
    print("🔄 L'URL restera active tant que ce code tourne")
    print("💡 Pour une URL permanente, on passera à Netlify après")
    print("="*60)
    
    # Garder l'application active
    print("\n🔄 Application active... Appuie sur Ctrl+C pour arrêter")
    
    # Boucle pour maintenir l'application
    try:
        while True:
            time.sleep(1)
    ...
    print(f"❌ Erreur lors de la création de l'URL : {e}")
    print("💡 Assure-toi que ngrok est bien installé")
    print("🔄 Relance le code si nécessaire")
